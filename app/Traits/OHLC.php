...
trait OHLC
{
    ...
    public function markOHLC($ticker, $bf = false, $bf_pair = 'BTC/USD')
    {
        ...
        /** tick table update */
        $ins = \DB::insert("
            INSERT INTO cryptobot_ohlc_tick
        ...
        $last1m = \DB::table('cryptobot_ohlc_1m')->select(DB::raw('MAX(timeid) AS timeid'))
        ...
        if ($last1timeid < $timeid) {
            ...
            $accum1ma = \DB::table('cryptobot_ohlc_tick')->select(DB::raw('MAX(high) as high, MIN(low) as low'))
                ->where('instrument', $bf_pair)
            ...
            $accum1mb = \DB::table('cryptobot_ohlc_tick')->select(DB::raw('open AS open'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last1timeids)
                ->where('timeid', '<=', ($last1timeids + 59))
                ->limit(1)
            ...
            $accum1mc = \DB::table('cryptobot_ohlc_tick')->select(DB::raw('close AS close'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last1timeids)
                ->where('timeid', '<=', ($last1timeids + 59))
                ->orderBy('ctime', 'desc')
        ...
        $last5m = \DB::table('cryptobot_ohlc_5m')->select(DB::raw('MAX(timeid) AS timeid'))
            ->where('instrument', $bf_pair)
            ->get();
        ...
        if ($last5timeid < $timeid) {
            ...
            $accum5ma = \DB::table('cryptobot_ohlc_1m')->select(DB::raw('MAX(high) as high, MIN(low) as low'))
                ->where('instrument', $bf_pair)
            ...
            $accum5mb = \DB::table('cryptobot_ohlc_1m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last5timeids)
                ->where('timeid', '<=', ($timeid))
                ->limit(1)
            ...
            $accum5mc = \DB::table('cryptobot_ohlc_1m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last5timeids)
                ->where('timeid', '<=', ($timeid))
                ->orderBy('ctime', 'desc')
        ...
        $last15m = \DB::table('cryptobot_ohlc_15m')->select(DB::raw('MAX(timeid) AS timeid'))
            ->where('instrument', $bf_pair)
            ->get();
        ...
        if ($last15timeid < $timeid) {
            ...
            $accum15ma = \DB::table('cryptobot_ohlc_5m')->select(DB::raw('MAX(high) as high, MIN(low) as low'))
                ->where('instrument', $bf_pair)
            ...
            $accum15mb = \DB::table('cryptobot_ohlc_5m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last15timeids)
                ->where('timeid', '<=', ($timeid))
                ->limit(1)
            ...
            $accum15mc = \DB::table('cryptobot_ohlc_5m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last15timeids)
                ->where('timeid', '<=', ($timeid))
                ->orderBy('ctime', 'desc')
            ...
            if ($open15 && $close15 && $low15 && $high15) {
                $ins = \DB::insert("
            INSERT INTO cryptobot_ohlc_15m 
            (`instrument`, `timeid`, `open`, `high`, `low`, `close`, `volume`)
            VALUES
        ...
        $last30m = \DB::table('cryptobot_ohlc_30m')->select(DB::raw('MAX(timeid) AS timeid'))
            ->where('instrument', $bf_pair)
            ->get();
        ...
        if ($last30timeid < $timeid) {
            ...
            $accum30ma = \DB::table('cryptobot_ohlc_15m')->select(DB::raw('MAX(high) as high, MIN(low) as low'))
                ->where('instrument', $bf_pair)
            ...
            $accum30mb = \DB::table('cryptobot_ohlc_15m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last30timeids)
                ->where('timeid', '<=', ($timeid))
                ->limit(1)
            ...
            $accum30mc = \DB::table('cryptobot_ohlc_15m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last30timeids)
                ->where('timeid', '<=', ($timeid))
                ->orderBy('ctime', 'desc')
            ...
            if ($open30 && $close30 && $low30 && $high30) {
                $ins = \DB::insert("
            INSERT INTO cryptobot_ohlc_30m 
            (`instrument`, `timeid`, `open`, `high`, `low`, `close`, `volume`)
            VALUES
        ...
        $last60m = \DB::table('cryptobot_ohlc_1h')->select(DB::raw('MAX(timeid) AS timeid'))
            ->where('instrument', $bf_pair)
            ->get();
        ...
        if ($last60timeid < $timeid) {
            ...
            $accum60ma = \DB::table('cryptobot_ohlc_30m')->select(DB::raw('MAX(high) as high, MIN(low) as low'))
                ->where('instrument', $bf_pair)
            ...
            $accum60mb = \DB::table('cryptobot_ohlc_30m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last60timeids)
                ->where('timeid', '<=', ($timeid))
                ->limit(1)
            ...
            $accum60mc = \DB::table('cryptobot_ohlc_30m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last60timeids)
                ->where('timeid', '<=', ($timeid))
                ->orderBy('ctime', 'desc')
    ...
    public function getRecentData($pair='BTC/USD', $limit=168, $day_data=false, $hour=12, $periodSize='1m', $returnRS=false)
    {
        ...
        $a = \DB::table('cryptobot_ohlc_'.$periodSize)
