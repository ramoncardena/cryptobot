...
class CoincapWebsocketCommand extends Command
{
     /**
     ...
     * @var string
     */
    protected $signature = 'cryptobot:websocket_coincap';
     ...
     * @var string
     */
    protected $description = 'Connect to the CoinCap websocket and store OHLC data';
    ...
    {   
        $client = new Client(new Version2X('http://socket.coincap.io'));
        
        $client->initialize();
        
        $client->emit('trades',['foo'=>'bar']);
        while (true) {
            $r = $client->read();
            if (!empty($r)) {
                var_dump($r);
